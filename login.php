...
if (isset($enviar)){
	...
	if(!empty($_POST['g-recaptcha-response'])){
		...
		if($resultCaptcha->success == 1){
			...
			$resultado = autenticausuario($login,$senha,"1");
			if(OCIExecute($resultado)){
				while(OCIFetch($resultado)){
					...
					$senhaAnt = OCIResult($resultado,8);
					$bloqueado = OCIResult($resultado,9);
				}
			}
	...
	if ($cont > 0 && $bloqueado == 0){
		...
		$_SESSION["senha"] = $senhaok;

		$reset = reset_fail_to_ban($login);
		oci_execute($reset, OCI_COMMIT_ON_SUCCESS);

		$log = log_acesso($login, $client_ip);
		oci_execute($log, OCI_COMMIT_ON_SUCCESS);
			...
			<div class="container theme-showcase" role="main">
				<div class='panel panel-danger'> 
					...
					<div class='panel-body' align='center'> 	
						<p align='center'>Usuario ou senha inválido</p><br><br>
						<p align='center'>$qtdAcesso</p><br><br>
						<a href='javascript:history.go(-1);'><button type='button' class='btn btn-default btn-sm'>
						<span class='glyphicon glyphicon-arrow-left' aria-hidden='true'></span> Voltar</button></a>
					</div> 
				</div>
			</div>
